...
public class HttpClientFactory {
    ...
    private HttpClientFactory() {
        ...
        LOG.info("HttpClientFactory创建成功，并添加默认的HttpClientBuilder, 名字为：Default .");
    }
    ...
    public HttpClientBuilder getBuilder(String builderName) {
        HttpClientBuilder builder = FACTORY_BUILDERS.get(builderName);
        if (builder == null) {
            LOG.warn("\"{}\" HttpClientBuilder 不存在，将使用默认HttpClientBuidler(\"{}\")", builderName, DEFAULT_BUILDER);
            builder = factory.getBuilder(DEFAULT_BUILDER);
        }
        return builder;
    }
    ...
    public class _HttpClient {
        ...
        public _HttpClient changeGetUrl(String url) {
            try {
                this.get.setURI(new URI(url));
            } catch (URISyntaxException e) {
                LOG.error(e.getMessage(), e);
                return this;
            }
        ...
        public _HttpClient changePostUrl(String url) {
            try {
                this.post.setURI(new URI(url));
            } catch (URISyntaxException e) {
                LOG.error(e.getMessage(), e);
                return this;
            }
